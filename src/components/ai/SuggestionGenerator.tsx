...
const SuggestionGenerator: React.FC<SuggestionGeneratorProps> = ({ formData, campaign }) => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      // Get the campaign data from either source
      const campaignData = campaign || formData;
      
      // Create a new mock suggestion based on the campaign data
      const newSuggestion: CampaignSuggestion = {
        id: `s${suggestions.length + 1}`,
        campaign_id: campaign?.id || 'draft',
        title: "Leverage Personal Stories",
        description: `Based on your ${campaignData?.category || ''} campaign "${campaignData?.title || ''}", consider adding testimonials from people who have benefited from your work. Personal stories create emotional connections with potential donors and can increase conversion rates by up to 30%.`,
        type: 'content',
